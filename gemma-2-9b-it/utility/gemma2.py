#root@nemo-gpu-export-job-worker-0-0:/workspace# cat /data/nvidia-pipeline/gemma2.py
...
class HFGemmaExporter(io.ModelConnector[Gemma2Model, "GemmaForCausalLM"]):
    ...
    def init(self, dtype=torch.bfloat16) -> "GemmaForCausalLM":
        """Initialize a HF GemmaForCausalLM instance."""
...
@io.model_exporter(Gemma2Model, "hf-peft")
class HFGemmaPEFTExporter(HFGemmaExporter):
    ...
    def apply(self, output_path: Path) -> Path:
        ...
        source, _ = self.nemo_load(str(self))
        target = self.init(dtype=torch_dtype_from_mcore_config(source.config))
        target = self.convert_state(source, target)
        target = target.cpu()
